...
if (isset($_GET['token'])) {
    ...
    if (isset($_SESSION['demo_reset_token']) && $_SESSION['demo_reset_token'] === $token && isset($_SESSION['demo_reset_email'])) {
        $validToken = true;
        $resetEmail = $_SESSION['demo_reset_email'];
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $validToken) {
    ...
    if (empty($passwordErr) && empty($confirmPasswordErr)) {
        ...
        $stmt = $conn->prepare("UPDATE user_sign_in SET password = ? WHERE email = ?");
        $stmt->bind_param("ss", $hashedPassword, $resetEmail);
        
        if ($stmt->execute()) {
            $successMsg = "Password successfully reset! Redirecting to sign in...";
